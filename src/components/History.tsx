...
export default function History({ getHistory, show, onClose }: { getHistory: () => Promise<HistoryItem[]>, show: boolean, onClose: () => void }) {
  ...
  return (
    <div className={twMerge("bg-transparent absolute bottom-0 h-screen w-full z-30 transition-[max-width] duration-300 flex overflow-hidden max-w-[0px]", show && "max-w-full")}>
      <div className="w-[80%] sm:w-[70%] md:w-[50%] max-w-[400px] h-full bg-primary flex flex-col py-4 shadow-2xl">
        ...
        ">
          {
            history.map(i => (
              <li
                ...
                className={twMerge("p-2 ml-4 pl-4 border-l-[3px] flex justify-between items-end", i.value >= 0 ? "text-good" : "text-danger", i.value >= 0 ? "border-good" : "border-danger")}>
